...
class EyeTrackingHeatmap:
    def __init__(self):
        ...
        self.frame_buffer = []
        self.fps = 30  # Adjusted for Mac
        self.frame_time = 1/self.fps
        self.last_frame_time = 0

        # Initialize webcam with Mac settings
    ...
    def start_recording(self):
        ...
        self.output_filename = f"eye_tracking_session_{current_time}.mov"
        ...
        # Initialize video writer with Mac-compatible codec
    ...
    def record_screen(self):
        ...
        while self.is_recording:
            ...
            if current_time >= next_frame_time:
                try:
                    # Mac-specific screen capture
                    screen = NSScreen.mainScreen()
                    frame = screen.frame()
                    region = (0, 0, int(frame.size.width), int(frame.size.height))
    ...
    def calibrate(self):
        """Improved calibration process for Mac"""
        ...
        for point_idx, (px, py) in enumerate(calibration_points):
            ...
            cv2.putText(calibration_screen, f"Look at the green dot ({point_idx + 1}/9)", 
    ...
    def check_permissions(self):
        """Check for required Mac permissions"""
        print("\nMac Permission Check:")
        print("Please ensure the following permissions are enabled in System Preferences → Security & Privacy → Privacy:")
        print("1. Camera access")
        print("2. Screen Recording access")
        print("3. Accessibility access (if needed)")
        input("\nPress Enter once permissions are enabled...")
